...
import { SyncEngine, IndexedDbStore, type ApiPort, type VaultPort } from "../src/plugin/sync";
import {
  digest,
  fromBase64,
  toBase64,
...
class Vault implements VaultPort {
...
class Server {
  docs = new Map<string, { doc: Y.Doc; file: FileRecord }>();
  ...
  api: ApiPort = {
    ...
    operate: async (operation) => {
      ...
      let value = this.docs.get(operation.fileId);
      if (!value) {
        value = {
          doc: new Y.Doc(),
          file: {
      ...
      const bytes = encode(value.doc.getText("content").toString());
      value.file.digest = await digest(bytes);
      value.file.size = bytes.length;
      value.file.revision = ++this.revision;
      const result = {
        opId: operation.opId,
        previousRevision,
        previousPathRevision,
