...
export class VaultSockets {
  ...
  broadcast(message: ServerMessage): void {
    ...
    for (const ws of this.state.getWebSockets()) {
      if (ws.readyState !== WebSocket.OPEN || this.closeExpired(ws)) continue;
      try {
        ws.send(data);
      } catch {
  ...
  async expire(): Promise<void> {
    ...
    for (const ws of this.state.getWebSockets()) this.closeExpired(ws);
  }
  ...
  private closeExpired(ws: WebSocket): boolean {
    ...
    if (attachment.expiresAt > Date.now()) return false;
