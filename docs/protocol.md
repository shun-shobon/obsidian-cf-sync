...
`/ws`はAccessの通常ログインを通さず、WorkerとDOが接続券を検証する。接続券は端末・Vaultに紐づき、30秒間有効で一度だけ使える。接続後の時間制限は設けず、Accessの認証期限だけでは既存接続を切断しない。端末失効時は既存接続も閉じる。HTTP操作と再接続には認証が必要である。

WebSocketはサーバーからの変更通知に使う。編集は250ms単位でHTTPへまとめ、通知を受けた端末はメタデータと本文を取得する。添付データをWebSocketには流さない。

無操作時の定期同期・heartbeat・強制再接続は行わない。ローカル編集・変更通知・起動・アプリ復帰・オンライン復帰・手動操作を契機に同期する。再接続ではWebSocketを開いてからsnapshotを取得し、その間の通知も処理する。通信失敗や切断を検知したときだけ、間隔を延ばして再試行する。

回線断を検知できず接続中に見える場合は、操作・復帰・手動同期までリモート変更の反映が遅れることがある。復帰時は接続を作り直し、ローカルの再走査とsnapshot照合で変更を回収する。
...
WebSocketの接続有無を理由にalarmを繰り返さない。未反映データ・未使用接続券・未参照添付の期限から次の処理時刻を決め、仕事がなければalarmを削除する。保存期限と添付掃除の期限はDOストレージに保持する。R2保存通知は保存位置が進んだときだけ送る。
